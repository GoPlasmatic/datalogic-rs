use datalogic_rs::{DataArena, IntoLogic, evaluate};
...
fn main() {
    ...
    println!("Benchmarking simple map operation ({} iterations):", iterations);
    ...
    let total_ms = duration.as_secs() as f64 * 1000.0 + duration.subsec_nanos() as f64 / 1_000_000.0;
    let avg_ns = (duration.as_secs() as f64 * 1_000_000_000.0 + duration.subsec_nanos() as f64) / iterations as f64;
    let ops_per_sec = iterations as f64 / (duration.as_secs() as f64 + duration.subsec_nanos() as f64 / 1_000_000_000.0);
    ...
    println!("\nBenchmarking complex operation (filter + map + reduce) ({} iterations):", iterations);
    let start = Instant::now();
