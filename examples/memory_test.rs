...
fn main() {
    ...
    println!("Logic arena memory (unchanged): {} bytes", logic_arena.memory_usage());
    println!("Arena resets performed: {}", reset_count);
    println!("Max memory growth between resets: {} bytes", max_memory_between_resets);
    ...
    if max_memory_between_resets <= 33554432 { // 32MB is typical arena chunk size
        println!("✅ PASS: No memory leak detected!");
        println!("   Arena reached steady state at {} bytes", max_memory_between_resets);
    ...
    if extra_diff > 0 {
        ...
        println!("Average growth per evaluation: {:.2} bytes", growth_per_eval);
