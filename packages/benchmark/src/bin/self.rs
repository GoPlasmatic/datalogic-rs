...
//! - `evaluate_ref` returns a borrowed `&DataValue<'a>` tied to the session
...
//!   `session.reset()` after every `evaluate_ref` so peak memory stays
...
//! shape exactly (same `evaluate_ref` + `reset` cadence), so the bump
...
fn benchmark_suite(engine: &Engine, suite_name: &str) -> Option<(SuiteResult, usize)> {
    ...
    // Warm-up — same per-iteration `evaluate_ref` + `reset` shape as the
    ...
    for (rule, data) in compiled.iter().zip(inputs.iter()) {
        for _ in 0..ITERATIONS {
            let _ = session.evaluate_ref(rule, *data);
            session.reset();
        }
    }
