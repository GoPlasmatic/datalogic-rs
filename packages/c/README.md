...
C ABI for the [`datalogic-rs`](../core) JSONLogic engine. This crate is
the canonical FFI boundary the language bindings under `packages/go`,
`packages/php`, `packages/jvm`, etc. consume.
...
|---|---|---|
| Go | `packages/go/` | `cgo` over the cdylib (planned, next) |
| PHP | `packages/php/` | PHP FFI (planned) |
| JVM | `packages/jvm/` | JNA / JNR-FFI / JNI (planned) |

The Python (`packages/python/`) and WASM (`packages/wasm/`) bindings
