...
use crate::node::{
    CompileCtx, CompiledLogic, CompiledMissingArg, CompiledMissingData, CompiledMissingMin,
    CompiledMissingPaths, CompiledMissingSomeData, CompiledNode, MetadataHint, PathSegment,
    ReduceHint, node_is_static,
};
...
impl CompiledLogic {
            ...
            Value::Object(obj) if obj.len() == 1 => {
                ...
                if let Ok(opcode) = op_name.parse::<OpCode>() {
                    ...
                    if opcode == OpCode::Missing {
                        return Ok(Self::compile_missing(args, ctx));
                    }
                    if opcode == OpCode::MissingSome {
                        return Ok(Self::compile_missing_some(args, ctx));
                    }
    ...
    fn compile_missing(args: Box<[CompiledNode]>, ctx: &mut CompileCtx) -> CompiledNode {
        let mapped: Vec<CompiledMissingArg> = args
            ...
            .map(|arg| match &arg {
                CompiledNode::Value {
                    ...
                    let segments = Self::parse_path_segments(s).into_boxed_slice();
    ...
    fn compile_missing_some(args: Box<[CompiledNode]>, ctx: &mut CompileCtx) -> CompiledNode {
