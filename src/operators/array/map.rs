...
fn map_var_fast_path<'a>(
    ...
    Some(arena.alloc(ArenaValue::Array(results.into_bump_slice())))
}
...
fn map_arena_bridge<'a>(
    ...
    match input {
        ArenaValue::Object(pairs) => map_bridge_object(pairs, body, actx, engine, arena),
        ArenaValue::Array(items) => map_bridge_array(items, body, actx, engine, arena),
...
fn map_bridge_object<'a>(
    ...
    let mut results = bvec::<ArenaValue<'a>>(arena, pairs.len());
    let mut guard = IterGuard::new(actx);
...
fn map_bridge_array<'a>(
    ...
    let mut results = bvec::<ArenaValue<'a>>(arena, items.len());
    let mut guard = IterGuard::new(actx);
