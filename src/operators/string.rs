...
pub(crate) fn evaluate_substr_arena<'a>(
    ...
    let start: i64 = if args.len() > 1 {
        if let CompiledNode::Value { value, .. } = &args[1] {
            ...
            engine
                .evaluate_arena_node(&args[1], actx, context, arena)?
                .as_i64()
                .unwrap_or(0)
        }
    } else {
    ...
    let length: Option<i64> = if args.len() > 2 {
        if let CompiledNode::Value { value, .. } = &args[2] {
            ...
            engine
                .evaluate_arena_node(&args[2], actx, context, arena)?
                .as_i64()
        }
    } else {
        None
...
pub(crate) fn evaluate_in_arena<'a>(
    ...
    let needle = engine.evaluate_arena_node(&args[0], actx, context, arena)?;
    let haystack = engine.evaluate_arena_node(&args[1], actx, context, arena)?;
...
pub(crate) fn evaluate_split_arena<'a>(
    ...
    } = &args[1]
    {
        ...
        let av = engine.evaluate_arena_node(&args[1], actx, context, arena)?;
        match av {
            ArenaValue::String(s) => *s,
            ArenaValue::InputRef(Value::String(s)) => arena.alloc_str(s.as_str()),
