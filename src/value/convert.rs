...
pub fn hash_map_to_data_value<'a, V>(
    ...
    let entries: Vec<(&'a str, DataValue<'a>)> = map.iter()
        .map(|(key, value)| {
            let interned_key = arena.intern_str(key);
...
mod tests {
    ...
    fn test_hash_map_conversion() {
        ...
        if let DataValue::Object(entries) = data_value {
            ...
            assert!(found_a && found_b && found_c, "Not all expected keys were found");
        } else {
    ...
    fn test_json_to_data_value() {
        ...
        assert!(found_name && found_age && found_is_active, "Not all expected keys were found");
    }
    ...
    fn test_data_value_to_json() {
        ...
        // Create a DataValue
        let data_value = DataValue::object(&arena, &[
                ...
                match *key {
                    "user1" => {
                        if let DataValue::Object(user_entries) = value {
                            ...
                            assert!(found_name && found_age, "Not all expected user fields were found");
                        } else {
