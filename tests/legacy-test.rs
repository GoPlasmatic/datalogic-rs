...
fn test_legacy_operators() {
    ...
    let index_json_str = fs::read_to_string(index_file_path).expect("Failed to read index file");

    // Parse the index file to get the list of test files
    let test_files: Vec<String> =
        serde_json::from_str(&index_json_str).expect("Failed to parse index.json");
    ...
    for test_file in test_files {
        ...
        println!(
            ...
            test_file, file_passed, file_failed
        );
    }
